...
#+RESULTS:
: #2A((0.0 1.0) (-1.0 0.0))
** Passing lisp objects to python: =register-handler=
...
handler function needs to be registered. This function will be called when a
python function attempts to access attributes of an object (=__getattr__=
method).

*Note* This part is somewhat experimental. A nicer interface may be possible, but
for now a user-supplied function provides a flexible solution. It does not yet
take account of inheritance, but that should be possible with a bit more thought.
...
(let ((instance (make-instance 'test-class :value 21)))
  ...
  ;; Register a handler for this class of object, by passing in an instance.
  ;; Any objects with the same CLASS-OF class object will use this handler
  (py4cl:register-handler instance
...
For classes a handler can be registered without passing in an object,
using [[http://clhs.lisp.se/Body/f_find_c.htm][find-class]]:
#+BEGIN_SRC lisp
;; Define a class with some slots
(defclass test-class ()
  ((value :initarg :value)))
